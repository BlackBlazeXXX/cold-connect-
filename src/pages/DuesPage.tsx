...
import { useContacts, FollowUpStage, filterContactsByStage, getFollowUpStage, getDailyNewLeads, markLeadsAsShown, getRemainingLeadsCount } from '../hooks/useContacts';
...
const VALID_TABS: Record<string, 'new' | 'followup'> = {
  new: 'new',
  followup: 'followup',
  follow_ups: 'followup',
  followups: 'followup',
...
export const DuesPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'new' | 'followup'>(initialTab);
  ...
  const currentContacts = activeTab === 'new' ? newLeads : followUps;
      ...
      {(currentContacts.length > 0 || selectedIds.size > 0) && (
        <Card className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-4 flex-wrap">
            <Badge
              variant={activeTab === 'new' ? 'success' : getStageBadgeVariant(activeStage)}
              size="sm"
        ...
        {currentContacts.length === 0 ? (
          <div className="py-16 px-6 text-center">
            ...
            <h3 className="text-lg font-medium text-white mb-1">
              {activeTab === 'new' ? 'No New Leads' : `No ${getStageLabel(activeStage)} Contacts`}
            </h3>
